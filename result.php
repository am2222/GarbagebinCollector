...
class GetProductInfo {
    ...
    function redeem() {
     ...
     if(isset($_GET["shift"])&&isset($_GET["car"]))
		{
 ...
 FROM (SELECT 'Feature' As type
    ...
    , row_to_json((lg.id, lg.cost, lg.reount)) As properties
   FROM result As lg where shift=? and car=? ) As f )  As fc;");
  
 /*   $stmt = $this->db->prepare("SELECT *,st_asgeojson(geom) AS geojson FROM pgr_fromAtoB('roads',?,?,?,?)"); */
			

          $stmt->execute(array($_GET["shift"],$_GET["car"]));	
		}
